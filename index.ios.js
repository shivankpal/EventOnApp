...
import { AppRegistry,BackHandler, ToastAndroid,Platform,AsyncStorage,Linking,View,Text,Image,Animated,Easing,TouchableWithoutFeedback,ActivityIndicator} from 'react-native';
...
const imageSource = {
      app_icon: require("./app/Assets/Img/app_icon.png"),
};
export default class EventOnApp extends Component {
  ...
  getRef = () => {
    AsyncStorage.getItem('USER').then((user)=>{
        if(user==null)
        {
           fetch('https://api.eventonapp.com/api/referrer/IOS', { method: 'GET' }).then((response) => response.json()).then((responseJson) => {
    ...
    AsyncStorage.getItem('DEVICETOKEN').then((devicetoken) => {
        if(devicetoken!=null){
          AsyncStorage.getItem('USER').then((user) => {
              if(user!=null){
                let u = JSON.parse(user);
                var url = 'https://api.eventonapp.com/api/deviceToken/'+u.id+"?device_id="+DeviceInfo.getUniqueID()+"&device_type=IOS&device_token="+devicetoken;
  ...
  handlePushNotification = () => {
      ...
      PushNotification.configure({
          ...
          onNotification: function(notification) {
              ...
              {
                  sudothis.setState({notification:notification.message},()=>{
                      Animated.timing(
                            sudothis.state.height,
                            {
                              toValue: 100,
                              duration: 300,
                      ...
                      if(user!=null)
                      {
                          AsyncStorage.getItem('MYEVENTS').then((events)=>{
                              if(events!=null)
                              {
                                  ...
                                  e.map((o,i)=>{
                                      if(o.id==notification.message.event_id)
                                      {
                                          var deeplink = [];
                                          deeplink[0] = o.id;
                                          deeplink[1] = notification.message.landing;
            ...
            if(user!=null)
            {
                AsyncStorage.getItem('MYEVENTS').then((events)=>{
                    if(events!=null)
                    {
                        var e = JSON.parse(events);
                        e.map((o,i)=>{
                            if(o.id==this.state.notification.event_id)
                            {
                                var deeplink = [];
                                deeplink[0] = o.id;
                                deeplink[1] = this.state.notification.landing;
                                this.navigator.replace( { id:'SudoSplash', user:JSON.parse(user), topevent:o, deeplink:deeplink } )
                            }
                        })
                    }
                })
  ...
  render() {
    ...
    return (
      <View style={{flex:1}}>
          <Animated.View  style={{height:this.state.height,overflow:'hidden',width:'100%',zIndex:1000,position:'absolute',top:0,left:0,right:0,backgroundColor:'rgba(0,0,0,0.9)'}}>
              <TouchableWithoutFeedback onPress={()=>{ this.ClickNotify() }}>
                <View style={{marginTop:22,flexDirection:'row',alignItems:'center',paddingHorizontal:10}}>
                  <Image source={imageSource.app_icon} style={{width:50,height:50,borderRadius:25,marginRight:10}}/>
                  <View style={{alignItems:'flex-start',flex:1}}>
                      <Text style={{color:'#EAEAEA',fontSize:18,fontFamily:'Roboto-Medium'}}>{this.state.notification.title}</Text>
                      <Text style={{color:'#EAEAEA',fontSize:12,fontFamily:'Roboto-Regular'}} numberOfLines={2}>{this.state.notification.body}</Text>
                  </View>
                </View>
