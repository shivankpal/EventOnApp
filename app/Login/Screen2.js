...
export default class Screen2 extends Component {
    ...
    pushToScreen3 = () => {
      if(this.state.data.length)
      {
          this.props.navigator.push({ id: 'Screen3', data:[this.state.data,this.state.prevData]  })
      }
      else
      {
          ...
          <View style={{flex:1,backgroundColor:'#292E39',justifyContent:'center'}}>
            <CustomStatusBar backgroundColor="#292E39" barStyle="light-content"/>
              <View style={{flex:0.2,justifyContent:'center',alignItems:'center',borderBottomWidth:1,borderColor:'#353a4c'}}>
                  ...
                  <TouchableOpacity onPress={()=>this.pushToScreen3()} style={{position:'absolute',top:0,right:0,padding:15}}>
              ...
              <ScrollView style={{marginTop:10}} contentContainerStyle={{flexDirection:'row',flexWrap:'wrap'}}>
              {
                  this.state.buttons.map((o,i) => {
                    ...
                    return (
                      <TouchableOpacity key={i} ref={'TOUCH_'+o.id} onPress={()=>{this.toggleTick(o.id)}}  style={{borderWidth:1,borderColor:'#353a4c',width:nwidth,height:nwidth/2,justifyContent:'center',alignItems:'center',margin:10,padding:10}}>
