...
export default class Screen4 extends Component {
  ...
  pushToMain = async () => {
    ...
    {
        var r = this.state.prevData;
        r = r.concat(this.inputData);
        this.saveProfiling(r);
    }
    else
    {
  ...
  saveProfiling = async (attr) => {
    await AsyncStorage.getItem('USER').then((user) => {
      if(user!=null)
      {
          let t = JSON.parse(user);
          fetch('https://api.eventonapp.com/api/saveProfiling/'+t.id, {
             method: 'POST',
             body: JSON.stringify({
               attribute: attr,
             }),
                      ...
                      if(events!=null) {
                        this.setState({events: JSON.parse(events)}, () => {
                            AsyncStorage.setItem('PROFILE','true', () => {
                                this.props.navigator.resetTo({id:'Index', user:this.state.user, topevent:this.state.topevent});
                            })
                         })
                      }
                      else{
        ...
        <View style={{flex:1,backgroundColor:'#292E39',justifyContent:'center'}}>
            <CustomStatusBar backgroundColor="#292E39" barStyle="light-content"/>
              <View style={{flex:0.2,justifyContent:'center',alignItems:'center',borderBottomWidth:1,borderColor:'#353a4c'}}>
                  ...
                  <TouchableOpacity onPress={()=>this.pushToMain()} style={{position:'absolute',top:0,right:0,padding:15}}>
