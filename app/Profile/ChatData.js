...
export default class ChatData extends Component {
    ...
    componentWillMount(){
        if((this.props.chats.length==0) && this.props.template)
        {
            this.setState({message: "Hi "+this.state.touser.name+",\nIt is very nice to meet you. I'd be happy to connect. " });
        }
    }
    componentDidMount(){
        this.execFunct = setTimeout(()=>{ this.receiveMsg() },3000);
    }
    componentWillReceiveProps(nextProps){
        this.setState({message: "" });
        if((nextProps.chats.length==0) && nextProps.template)
        {
            this.setState({message: "Hi "+this.state.touser.name+",\nIt is very nice to meet you. I'd be happy to connect. " });
        }
        else
    ...
    render () {
      ...
      return (
        <View style={{flex:1}}>
          ...
          <Animated.View style={{overflow:'hidden',opacity:this.height,height:this.height,backgroundColor:'#FFF',borderTopColor:'#EAEAEA',borderTopWidth:1,justifyContent:'center'}}>
            <ScrollView alwaysBounceVertical={false} keyboardShouldPersistTaps={'always'} >
              <View style={{flexDirection:'row',flexWrap:'wrap',flex:1,padding:10}}>
                ...
                <TouchableOpacity onPress={()=> this.addToText('Hi '+this.state.touser.name+', I think we share common interests. I would love to setup a time to chat at the event.') }><Text style={styles.quick}>Hi {this.state.touser.name}, I think we share common interests. I would love to setup a time to chat at the event.</Text></TouchableOpacity>
                <TouchableOpacity onPress={()=> this.addToText('Hi '+this.state.touser.name+', Its very nice to meet you. I\'d be happy to connect.') }><Text style={styles.quick}>Hi {this.state.touser.name}, Its very nice to meet you. I'd be happy to connect.</Text></TouchableOpacity>
                <TouchableOpacity onPress={()=> this.addToText('Hi '+this.state.touser.name+', I\'m here at the event. I\'d love to meet up to discuss a possible collaboration  opportunity. Hope to see you soon. .') }><Text style={styles.quick}>Hi {this.state.touser.name}, I'm here at the event. I'd love to meet up to discuss a possible collaboration  opportunity. Hope to see you soon.</Text></TouchableOpacity>
              </View>
            </ScrollView>
          </Animated.View>
          <View style={{borderTopColor:'#EAEAEA',borderTopWidth:1,flexDirection:'row',alignItems:'flex-end',backgroundColor:'#FFF'}}>
              <TextInput
