...
export default class PeopleTabview extends Component {
    ...
    render () {
      ...
      {
          let c = this.state.comment.length;
          let w = 1;
          if(c > 0){
            return (
              <View>
                {
                   this.state.comment.map((o,i) => {
                        ...
                        return (
                              <View key={i} style={{borderBottomWidth:w,borderBottomColor:'#EEE',paddingVertical:10,flexDirection:'row'}}>
                                  <Image source={{uri:o.image_small}} style={{width:60,height:60,borderRadius:30,marginRight:10}}/>
                                  <View style={{flex:1}}>
                                      <View style={{flex:1,flexDirection:'row'}}>
                                          <Text style={{flex:1,fontFamily:'Roboto-Medium',fontSize:14,color:'#000'}}>{o.title}</Text>
                                          <Text style={{fontFamily:'Roboto-Regular',fontSize:10,color:'#999'}}>{o.ago}</Text>
                                      </View>
                                      <View>
                                          <Text style={{fontFamily:'Roboto-Regular',fontSize:11,color:'#444'}} ref={e => (this.readmore[o.id] = e)}  numberOfLines={2}>{o.content}</Text>
                                          {
                                             ...
                                             <View style={{alignItems:'flex-end'}} ref={e => (this.readmorebtn[o.id] = e)}>
                                                 <TouchableOpacity onPress={()=>this.expandMore(o.id)} style={{justifyContent:'center',alignItems:'center'}}>
                                                     <Text style={{fontSize:11,fontFamily:'Roboto-Medium',color:'#444'}}>Read More</Text>
                                                 </TouchableOpacity>
                                             </View>
                                             :
                                             null
